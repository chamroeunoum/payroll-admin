...
const actions = {
  ...
  async start ({ state, commit, rootState },params) {
    return await crud.create(state.server+"/"+state.model.module+'/startmaintenance',params)
  },
  ...
  async total ({ state, commit, rootState },params) {
    return await crud.read(state.server+"/dashboard/"+state.model.module+"/total")
  },
  async useService ({ state, commit, rootState },params) {
    return await crud.update(state.server+"/"+state.model.module+'/useservice',params)
  },
  async meetingRooms ({ state, commit, rootState },params) {
    return await crud.list(state.server + "/meetingrooms?" + new URLSearchParams({
        search: params.search ,
        perPage: params.perPage ,
        page: params.page
      }).toString()
  ...
  async toggleFacility({state, commit, rootState}, params){
    return await crud.update(import.meta.env.VITE_API_SERVER+"/"+state.model.module+"/togglefacility",params)
  },
  async toggleService({state, commit, rootState}, params){
    return await crud.update(import.meta.env.VITE_API_SERVER+"/"+state.model.module+"/toggleservice",params)
  },
  async savefoods ({ state, commit, rootState },params) {
    return await crud.create(state.server+"/"+state.model.module+'/savefoods',params)
  },
  async startmaintenance ({ state, commit, rootState },params) {
    return await crud.create(state.server+'/maintenances/startmaintenance',params)
  },
  async stopmaintenance ({ state, commit, rootState },params) {
    return await crud.update(state.server+'/maintenances/stopmaintenance',params)
  },
  async saveexpense ({ state, commit, rootState },params) {
    return await crud.create(state.server+"/"+state.model.module+'/savexpense',params)
  },

  async invoices ({ state, commit, rootState },params) {
    return await crud.list(
      state.server+"/"+state.model.module + '/' + params.id + "/invoices?"+ new URLSearchParams({
        cid: params.cid ,
        year: params.year ,
        month: params.month
      }).toString()
  ...
  async contracts ({ state, commit, rootState },params) {
    return await crud.list(
      state.server+"/"+state.model.module + '/' + params.id +  "/contracts"
    )
  }
}
