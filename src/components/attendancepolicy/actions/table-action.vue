...
export default {
  name: "AttendancePolicyTableActions",
  props: {
    model: { type: Object },
    record: { type: Object },
  ...
  setup(props){
    ...
    function deleteRecord(record){
      ...
      dialog.warning({
        title: "លុបទិន្នន័យ",
        content: "តើអ្នកចង់លុបទិន្នន័យនេះមែនទេ?",
        positiveText: 'បាទ / ចាស',
        negativeText: 'ទេ',
        onPositiveClick: () => {
          store.dispatch(props.model.name+'/delete',{id:record.id}).then( res => {
            if( res.data.ok ){
              notify.success({ title: 'លុប', description: 'លុបបានរួចរាល់។', duration: 3000 })
              closeActions(1)
            }
    ...
    function setAttendanceType(attnType){
      /**
       * `attendance_policies.type` is the policy kind (allowance/deduction) and
       * is constrained to those two values, so it must not receive an attendance
       * code. The buttons choose which attendance type the policy belongs to,
       * which is `attendance_type_id`; the API resolves the code to its id.
       */
      store.dispatch(props.model.name+'/update',{ id: props.record.id, attendance_type: attnType }).then( res => {
        closeActions(1)
    ...
    return { show, toggleActions, closeActions, deleteRecord, setAttendanceType, showDetail }
  }
}
</script>
<style scoped>
