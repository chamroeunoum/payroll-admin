  ...
  <!-- Top action panel of crud -->
          ...
          <table class="vcb-table " >
            ...
            <tbody>
              <tr v-for="(record, index) in table.records.matched" :key='index' >
                ...
                <td>{{ record.title }}</td>
                <td>{{ record.type == 'allowance' ? 'បន្ថែម' : 'កាត់' }}</td>
                <td>{{ record.period }}</td>
                <td>{{ record.attendance_type_id }}</td>
                <td class="relative ">
    ...
    <div class="fixed left-0 right-0 bottom-1 flex flex-wrap" >
      <div class="vcb-table-pagination bg-blue-300 mx-auto">
        <div class="vcb-table-pagination-info" >{{ table.pagination.totalRecords > 0 ? table.pagination.totalRecords + " ឈ្មោះ" : "" }}</div>
