<template>
  ...
  <!-- Top action panel of crud -->
      ...
      <div class="flex-grow action-buttons flex-row-reverse flex">
        ...
        <div class="mt-1 mr-2 flex">
          <div class=" mx-2" >
            <n-button 
              icon-placement="left"
              :disabled="registerEmployerCodes.length<=0||uploading==true" type="primary" @click="registerAttendances"
              >
              <template #icon>
                <NIcon>
    ...
    <div class="mt-4 mb-24 relative">
      <div v-if="unregisterEmployerCodes.length" class="w-full my-1 mb-2 py-2 text-left text-md text-red-600 " >{{ 'មានមន្ត្រីដែលមិនទាន់បានចុះឈ្មោះចំនួន៖ ' + unregisterEmployerCodes.length + ' នៃតួលេខសរុប ' + rows.length + ' ។ សូមពិនិត្យបន្ទាប់ក្រហមខាងក្រោម។' }}</div>
...
export default {
  ...
  setup(){
    ...
    const registerEmployerCodes = ref([])
    const unregisterEmployerCodes = ref([])
    function handleExcelFile(event){
      const files = event.target.files
      rows.value = []
      columns.value = []
      filteredRows.value = [] 
      registerEmployerCodes.value = []
      unregisterEmployerCodes.value = []
    ...
    function reformatData( jsonData ){
      ...
      return jsonData.map( (row) => {
        ...
        if( row[11] == true ){
          registerEmployerCodes.value.push( row )
        }else{
          unregisterEmployerCodes.value.push( row )
        }
    ...
    function registerAttendances(){
      if( filteredRows.value.length > 0 ){        
        ...
        store.dispatch( 'attendance/importAttantances', { attendances: registerEmployerCodes.value } ).then( res => {
          if( res.data.ok == true ){
            rows.value = []
            columns.value = []
            filteredRows.value = [] 
    ...
    return {
      ...
      filteredRows ,
      search ,
      unregisterEmployerCodes ,
      registerEmployerCodes ,
      uploading ,
      /**
       * Functions
       */
