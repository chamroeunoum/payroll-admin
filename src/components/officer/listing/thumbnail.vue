        ...
        <div v-if="Array.isArray( table.records.matched ) && table.records.matched.length > 0 " class="vcb-thumbnail mb-12" >
          <div v-for="(record, index) in table.records.matched" :key='index' class="item" >
            <div class="content" >
              ...
              <div class="flex flex-wrap rounded-b-lg absolute left-0 bottom-0 right-0 h-20 bg-white/90" >
                <div class="w-full flex flex-wrap" >
                  <div class="w-full text-center font-moul pt-2 " >{{ 
                    record.countesy != undefined && record.countesy != null
                    ? record.countesy.name
                    : '' 
                    }} {{ 
                      record.people != undefined && record.people != null 
                      ? record.people.lastname + " " + record.people.firstname + ' - ' + record.people.enlastname + " " + record.people.enfirstname
                      : '' 
                  }}</div>
                </div>
...
.vcb-thumbnail .item .content {
  @apply hover:shadow duration-500 p-4 pt-8 relative hover:scale-105 transform-gpu bg-white hover:bg-yellow-100 rounded-lg;
  min-height: 10rem;
}
.vcb-thumbnail .item .content .image {
  @apply p-2 absolute left-0 top-0 right-0 bottom-0 flex-none mx-auto overflow-hidden rounded-lg ;
}
.vcb-filters-panel {
