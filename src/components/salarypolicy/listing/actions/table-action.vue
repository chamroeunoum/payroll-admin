<template>
  <div class="actions" >
    <Transition name="slide-fade" >
      <div v-if="show" class="panel" >
        <!-- <n-tooltip trigger="hover">
          <template #trigger>
            <svg class="action text-blue-500" @click="showServiceModal(record)" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 20 20"><g fill="none"><path d="M6 2a2 2 0 0 0-2 2v5h1V4a1 1 0 0 1 1-1h4v3.5A1.5 1.5 0 0 0 11.5 8H15v8a1 1 0 0 1-1 1h-2v1h2a2 2 0 0 0 2-2V7.414a1.5 1.5 0 0 0-.44-1.06l-3.914-3.915A1.5 1.5 0 0 0 10.586 2H6zm8.793 5H11.5a.5.5 0 0 1-.5-.5V3.207L14.793 7zM4 10.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V12h1a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h1v-1.5zm3 .5H5v1h2v-1zm-4 2a1 1 0 0 0-1 1v3a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1v-3a1 1 0 0 0-1-1H3z" fill="currentColor"></path></g></svg>
          </template>
          សេវារួមបញ្ចូល
        </n-tooltip> -->
        <!-- <n-tooltip trigger="hover">
    ...
    <service-form v-bind:model="model" v-bind:record="record" v-bind:show="serviceModal.show" :onClose="closeService" />
  </div>
</template>
<script>
...
import ServiceForm from '../../widgets/service.vue'
/**
 * CRUD component form
 */
export default {
  ...
  setup(props){
    ...
    function closeService(actionStatus){
    ...
    const serviceModal = reactive({show:false})
    function showServiceModal(record){
      // editRecord.id = record.id
      // editRecord.username = record.username
      // editRecord.firstname = record.firstname
      // editRecord.lastname = record.lastname
...
.vcb-thumbnail .actions {
  @apply flex flex-wrap justify-center absolute w-48 top-0 right-0 bottom-0;
}
.vcb-thumbnail .actions .action-toggle {
  @apply absolute top-2 right-2 w-5 h-5 p-0 rounded-full cursor-pointer ;
}
...
.vcb-thumbnail .actions .action {
  @apply cursor-pointer w-7 m-1 p-1 bg-white border border-gray-200 rounded ;
}
</style>